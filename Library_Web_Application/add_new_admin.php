...
if (isset($_POST['addAdmin'])) {
    ...
    if (empty($username) || empty($name) || empty($lastname) || empty($email) || empty($phone)) {
        ...
        if (mysqli_num_rows($checkResult) > 0) {
          ...
          $hashed_password = password_hash($password, PASSWORD_DEFAULT); // Hashimi i passwordit
          $sql = "INSERT INTO users (`role`, `username`, `password`, `name`, `lastname`, `email`, `phone`) 
          VALUES ('$default_role', '$username', '$hashed_password', '$name', '$lastname', '$email', '$phone')";
  

            $result = mysqli_query($conn, $sql);
            if ($result) {
                $mesazh = "Admini i shtua me sukses!";
                // header ("Location: admin_main_page.php");
            } else {
                $mesazh = "Admini i ri tashme ka akses mbi te dhena.";
            }
        }
    }
}
